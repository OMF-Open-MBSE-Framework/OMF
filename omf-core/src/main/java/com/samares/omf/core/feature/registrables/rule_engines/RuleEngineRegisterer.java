...
public class RuleEngineRegisterer extends FeatureItemRegisterer {
    /**
    ...
    public void register(MDFeature mdFeature) {
        try {
            mdFeature.getRuleEngines().forEach(this::addRuleEngine);
        }catch (Exception e){
    ...
    public void unregister(MDFeature mdFeature) {
        try {
            mdFeature.getRuleEngines().forEach(this::removeRuleEngine);
        }catch (Exception e){
            OMFErrorHandler.handleException(new FeatureException(
     ...
     */
    private void addRuleEngine(IFeatureRuleEngine ruleEngine){
     ...
     */
    private void removeRuleEngine(IFeatureRuleEngine ruleEngine){
            String category = ruleEngine.getCategory();
            IElementListener listener = getListenerFromCategory(category);
            HashMap<String, List<IRuleEngine>> ruleEngines = listener.getRuleEngineMap();

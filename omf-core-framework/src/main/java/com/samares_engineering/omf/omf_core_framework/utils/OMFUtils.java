...
//
    ...
    private static void openNode(BaseElement element, boolean appendSelection) {
        BrowserTabTree containmentTree = Application.getInstance().getMainFrame().getBrowser().getActiveTree();
        AtomicReference<TreePath> atomic = new AtomicReference<>();
        Utilities.invokeAndWaitOnDispatcher(() -> {
            boolean var6 = Node.isLocked();
            long var7 = Node.setLocked(false);

            try {
                atomic.set(containmentTree.openNode(element, true, appendSelection, true));
            } finally {
                Node.resetLock(var6, var7);
            }
     ...
     */
    public static void selectElementInContainmentTree(List<BaseElement> elements) {
        try {
            elements.forEach(element -> openNode(element, true));
        }catch (Exception e){
            throw new OMFCriticalException("SelectElementInContainmentTree failed cause: MagicDraw containment tree is not accessible", e);
        }
    }
